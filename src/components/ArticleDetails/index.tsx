...
const ArticleDetails: React.FC<ArticleDetailsType> = ({ setDetails, arts }) => {
  ...
  return (
    <div className="inset-0 z-50 fixed w-screen flex justify-center h-screen bg-black bg-opacity-5 text-black">
      <div className="bg-white w-[70rem] h-[45rem] rounded-md flex gap-6 -translate-y-9">
        <div className="flex-1 flex items-center justify-center">
          <div
            className={`bg-${arts?.image} bg-center bg-cover bg-no-repeat w-[30rem] h-[35rem] rounded-md ml-6`}
          ></div>
        </div>
        <div className="flex-1 flex flex-col bg-white p-8 rounded-md">
          <div className="flex flex-col flex-1 pt-8 gap-2">
            ...
            {ses
              .filter((ses) => ses.productId === arts?.id)
              .map((val, index) => {
                const stringDate = new Date(val.date + "");
                const formatter = new Intl.DateTimeFormat("fr-FR", {
                  dateStyle: "full",
                  timeStyle: "short",
                });
                const formattedDate = formatter.format(stringDate);
                return (
                  <div className="flex flex-col gap-2" key={index}>
                    <span>
                      {" "}
                      Date de l'enchère : <b>{formattedDate}</b>
                    </span>
                    <span>
                      {" "}
                      Durée : <b>{val.duree} minutes</b>
                    </span>
                  </div>
                );
              })}
          ...
          <div>
            {admin && (
              <form className="flex flex-1 justify-around px-4 flex-col">
                ...
                <div className="flex gap-12 mt-6">
                  <button
                    className="bg-purple-400 rounded-lg h-[2.5rem] flex items-center justify-center w-[12rem] cursor-pointer ml-10"
            ...
            {!admin && (
              <form className="flex flex-1 justify-around px-4 flex-col">
                <div className="flex gap-12 mt-6 justify-end">
                  <div
                    className="bg-purple-400 rounded-lg h-[2.5rem] flex items-center justify-center w-[7rem] cursor-pointer"
                    onClick={closing}
                  >
